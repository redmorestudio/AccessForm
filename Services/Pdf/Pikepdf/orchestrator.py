...
from Mcid.segment_bbox_extractor import SegmentBboxExtractor
from Mcid.spatial_mapper import SpatialMapper
...
class PikepdfOrchestrator:
            ...
            try:
                ...
                try:
                    element_mcid_map = self._build_element_mcid_map(
                        structure_tree, page_index, segments, operators
                    )
                except Exception as e:
                    logger.error(f"[ORCHESTRATOR-MCID] Failed to build MCID map on page {page_index + 1}: {e}", exc_info=True)
    ...
    def _build_element_mcid_map(
        ...
        # SPATIAL MAPPING: Use bounding box overlap (IoU) to match segments to nodes
        # This replaces the old sequential mapping (segment[i] -> node[i])

        nodes = structure_tree.get('nodes', [])
        logger.info(f"[ORCHESTRATOR] Starting spatial MCID mapping for {len(segments)} segments")

        # If no operators provided, fall back to sequential mapping
        if operators is None:
            logger.warning("[ORCHESTRATOR] No operators provided - falling back to sequential mapping")
            logger.warning("[ORCHESTRATOR] This means spatial mapping is disabled!")
            # Fallback: just map to root
            element_mcid_map = {}
            for segment in segments:
                if '/0' not in element_mcid_map:
                    element_mcid_map['/0'] = []
                element_mcid_map['/0'].append({
                    'page_index': page_index,
                    'mcid': segment['mcid']
                })
            return element_mcid_map

        # Step 1: Extract bounding boxes for segments
        logger.info(f"[ORCHESTRATOR] Extracting bounding boxes for segments")
        logger.info(f"[DEBUG] operators type: {type(operators)}, is None: {operators is None}, count: {len(operators) if operators else 'N/A'}")
        try:
            segments_with_bbox = SegmentBboxExtractor.extract_segment_bboxes(operators, segments)
        except Exception as e:
            logger.error(f"[ORCHESTRATOR] Bbox extraction failed: {e}", exc_info=True)
            raise

        # Add page_index to each segment
        for segment in segments_with_bbox:
            segment['page_index'] = page_index

        # Step 2: Use spatial mapper to match segments to nodes based on IoU
        logger.info(f"[ORCHESTRATOR] Mapping segments to nodes using spatial overlap (IoU)")
        element_mcid_map = SpatialMapper.map_segments_to_nodes(
            segments_with_bbox,
            nodes,
            iou_threshold=0.3  # Minimum 30% overlap required
        )

        # Log mapping statistics
        total_segments_mapped = sum(len(refs) for refs in element_mcid_map.values())
        fallback_count = len(element_mcid_map.get('/0', []))
        spatial_match_count = total_segments_mapped - fallback_count

        logger.info(f"[ORCHESTRATOR] Spatial mapping complete:")
        logger.info(f"  - Total segments: {len(segments)}")
        logger.info(f"  - Spatial matches: {spatial_match_count} ({spatial_match_count/len(segments)*100:.1f}%)")
        logger.info(f"  - Fallback to root: {fallback_count} ({fallback_count/len(segments)*100:.1f}%)")
        logger.info(f"  - Nodes with MCIDs: {len(element_mcid_map)}")
