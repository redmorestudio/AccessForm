...
namespace WordToPdfConverter.Services.Remediation.AI
{
    ...
    public class GptRemediationService : IRemediationService
    {
        ...
        private string BuildRemediationPrompt(string category, List<PdfUAViolation> violations)
        {
            ...
            sb.AppendLine();
            sb.AppendLine("IMPORTANT pikepdf API notes:");
            sb.AppendLine("- Use PascalCase for properties: pdf.Root (NOT pdf.root)");
            sb.AppendLine("- Use pikepdf.open() to open files");
            sb.AppendLine("- Use pdf.save() to save files");
            sb.AppendLine("- Metadata values MUST be strings: meta['pdfuaid:part'] = '1' (NOT integer 1)");
            sb.AppendLine("- Use register_xml_namespace() NOT register_namespace()");
            sb.AppendLine();
            sb.AppendLine("Working example - fixing PDF/UA metadata:");
            ...
            sb.AppendLine("    # Set values as STRINGS (not integers!)");
            sb.AppendLine("    meta['pdfuaid:part'] = '1'  # STRING '1', not integer 1");
            sb.AppendLine("    meta['dc:title'] = 'Accessible Document'");
            sb.AppendLine();
